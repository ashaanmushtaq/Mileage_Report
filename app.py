...
st.set_page_config(
    page_title="Mileage Processor | Dev by Ashaan", page_icon="🧾", layout="wide"
)
...
def generate_professional_excel(
  ...
  title_style.font = Font(
      name="Calibri", size=18, bold=True, color=COLORS["text_light"]
  )
  title_style.fill = PatternFill(
      start_color=COLORS["primary_dark"],
      end_color=COLORS["primary_dark"],
      fill_type="solid",
  )
  ...
  header_style.font = Font(
      name="Calibri", size=11, bold=True, color=COLORS["text_light"]
  )
  ...
  title_cell = ws.cell(
      row=1,
      column=1,
      value=f"{selected_city} VEHICLE MILEAGE EXECUTIVE REPORT",
  )
  ...
  ws.merge_cells(
      start_row=3, start_column=1, end_row=3, end_column=half_cols
  )
  ...
  ws.merge_cells(
      start_row=3, start_column=half_cols + 1, end_row=3, end_column=num_cols
  )
  r3_right = ws.cell(
      row=3,
      column=half_cols + 1,
      value=f"🚗 TOTAL FLEET (20-24h):  {total_valid_vehicles}",
  )
  r3_right.font = Font(
      name="Calibri", size=11, bold=True, color=COLORS["primary_dark"]
  )
  r3_right.alignment = center_align
  r3_right.fill = PatternFill(
      start_color=COLORS["header_bg"],
      end_color=COLORS["header_bg"],
      fill_type="solid",
  )
  ...
  r4_left = ws.cell(
      row=4, column=1, value="💻 DEVELOPED BY:  Muhammad Ashaan"
  )
  r4_left.font = Font(
      name="Calibri", size=10, bold=True, color=COLORS["primary_medium"]
  )
  r4_left.alignment = center_align

  ws.merge_cells(
      start_row=4, start_column=half_cols + 1, end_row=4, end_column=num_cols
  )
  r4_right = ws.cell(
      row=4, column=half_cols + 1, value=f"🏙️ URBAN FLEET:  {len(urban_rows)}"
  )
  ...
  ws.merge_cells(
      start_row=5, start_column=1, end_row=5, end_column=half_cols
  )
  r5_left = ws.cell(
      row=5, column=1, value="⏱️ FILTERED HOURS:  20 – 24 Hours"
  )
  r5_left.font = Font(name="Calibri", size=10, color=COLORS["primary_dark"])
  r5_left.alignment = center_align

  ws.merge_cells(
      start_row=5, start_column=half_cols + 1, end_row=5, end_column=num_cols
  )
  r5_right = ws.cell(
      row=5, column=half_cols + 1, value=f"🌾 RURAL FLEET:  {len(rural_rows)}"
  )
  ...
  sep2_cell.fill = PatternFill(
      start_color=COLORS["primary_light"],
  ...
  urban_bg_fill = PatternFill(
      start_color=COLORS["urban_bg"],
      end_color=COLORS["urban_bg"],
      fill_type="solid",
  )
  rural_bg_fill = PatternFill(
      start_color=COLORS["rural_bg"],
      end_color=COLORS["rural_bg"],
      fill_type="solid",
  )
  ...
  repeat_fill = PatternFill(
      start_color=COLORS["repeat_bg"],
      end_color=COLORS["repeat_bg"],
      fill_type="solid",
  )
  new_fill = PatternFill(
      start_color=COLORS["new_bg"],
      end_color=COLORS["new_bg"],
      fill_type="solid",
  )
...
with c_col1:
  ...
  if city_option == "Other (Custom)":
    selected_city = (
        st.text_input("Enter City Name:", value="Kamoke").strip().upper()
    )
  else:
...
with col_up1:
  current_file = st.file_uploader(
      "1️⃣ Upload TODAY'S Mileage Report (.xlsx)", type=["xlsx"]
  )

with col_up2:
  previous_file = st.file_uploader(
      "2️⃣ Upload PREVIOUS Mileage Report (.xlsx) - Optional", type=["xlsx"]
  )

with st.expander("⚙️ Optional: Master List Override"):
  rural_override_file = st.file_uploader(
      "Upload New Master List (.xlsx)", type=["xlsx"]
  )
...
if current_file:
  try:
    ...
    if h_idx == -1:
      st.error(
          "❌ Could not auto-detect 'Reg#' and 'Period' columns in Today's"
          " Mileage Report."
      )
    else:
      ...
      if previous_file:
        ...
        if p_h_idx != -1:
          ...
          for _, p_row in df_prev_data.iterrows():
            ...
            if 20.0 <= p_hrs <= 24.0:
              p_reg = (
                  str(p_row.iloc[p_reg_idx]).replace("\xa0", "").strip()
              )
      ...
      raw_headers = (
          df_current_raw.iloc[h_idx]
          .astype(str)
          .str.replace("\xa0", "", regex=False)
          .str.strip()
          .tolist()
      )
      has_sno = raw_headers[0].upper() in [
          "S.NO",
          "SR.#",
          "SR NO",
          "S#",
      ...
      for idx, row in df_data.iterrows():
        ...
        if 20.0 <= hrs <= 24.0:
          reg_val = (
              str(row.iloc[c_reg_idx]).replace("\xa0", "").strip()
              if pd.notna(row.iloc[c_reg_idx])
              else ""
          )

          if (
              not reg_val
              or reg_val in ["-", "NAN", "NONE", ""]
              or len(reg_val) < 2
          ):
      ...
      if repeated_vehicles:
        st.info(
            f"🔁 **Repeated Vehicles ({len(repeated_vehicles)}):**"
            f" {', '.join(repeated_vehicles)}"
        )
      if duplicate_regs:
        st.warning(
            f"⚠️ **Duplicate Vehicles ({len(duplicate_regs)}):**"
      ...
      st.download_button(
          ...
          data=output_buffer,
          file_name=(
              f"{selected_city}_Mileage_Report_{meta_date.replace('/', '_')}.xlsx"
          ),
          mime=(
